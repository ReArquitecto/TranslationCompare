...
class BibleNavigator {
  ...
  loadChaptersPage(book) {
    ...
    if (!this.bibleData || !this.bibleData[testament]) {
        console.error(`Bible data missing or does not contain the testament: ${testament}`);
        return;
    }
  ...
  loadTranslationPage(book, chapter, verse) {
    ...
    const content = `
      ...
      <ul>
        ${this.bibleData.Translations
          .map((translation) => `<li>${translation}: ${verse}</li>`)
          .join("")}      
      </ul>
      </div>`;
    this.appContent.innerHTML = content;
